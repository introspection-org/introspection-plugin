...
A Harbor suite is not a recipe resource. The package format declares agents, skills, extensions, prompts, judges, MCP, and runtime requirements — there is no eval or dataset declaration, and nothing in the recipe check validates one. An unknown manifest key is a diagnostic, not a pin, so never add an eval or Harbor key or describe a suite as declared by the recipe.

Version an accepted suite the way the repository versions everything else: keep it under a conventional directory and let its Git revision be the pin. Record which revision produced a result, and change the suite in its own commit rather than alongside agent behavior, so a score movement has one candidate cause.

Baselines belong to Harbor — establish them by running the suite against a frozen task version. The Introspection CLI has no eval, suite, or baseline command; its judge evaluation is calibration against labeled fixtures, not an offline suite run, so never substitute one for the other.

...
Treat durable coverage as an explicit handoff: the Evals capability owns the approved offline measurement contract, Harbor owns the environment, verifier, and run evidence, and the [Recipes capability](recipes.md) owns whatever the recipe repository keeps.

A Harbor suite is not a recipe resource. The recipe package format declares agents, skills, extensions, prompts, judges, MCP, and runtime requirements — there is no eval or dataset declaration, and nothing in the recipe check validates one. An eval directory in the repository is ordinary unvalidated content, so do not add an eval or Harbor key to the package manifest and do not describe a suite as declared by the recipe; an unknown manifest key is a diagnostic, not a pin.

Version an accepted suite the way the repository versions everything else: keep it in the recipe repository under a conventional directory, and let its Git revision be the pin. Record which revision produced a result alongside that result, and change the suite in its own commit rather than together with agent behavior, so a score movement has one candidate cause.

Baselines belong to Harbor. Establish and re-establish them by running the suite through Harbor against a frozen task version; the Introspection CLI has no eval, suite, or baseline command, and its judge evaluation is judge calibration against labeled fixtures rather than an offline suite run. Do not substitute one for the other.

...
Use this reference only after `$evals` selects Harbor as the lowest faithful evaluation layer. Use the installed Harbor `create-task` skill for exact task structure and commands.
...
Record the task version, agent and model, environment provider, attempt count, raw trial location, reward and reward details, infrastructure failures, and qualitative trace findings. Send new failure classes back to `$evals` before changing the agent.

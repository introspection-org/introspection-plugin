...
When an accepted task becomes durable recipe coverage, load `$introspection:recipes` first so its current-docs and latest-toolchain preflight runs. Then read the current Pi Recipe Evals documentation, pin the exact Harbor dataset version or Git revision in the recipe, and establish a new unchanged baseline through Pi recipe tooling. Keep the dataset outside the recipe and change an eval pin separately from agent behavior.

...
When an existing recipe is the approved starting point, resolve it from an explicit source or the machine-readable [Pi Recipes catalog](https://pi.recipes/catalog.json). Inspect its source, license text, providers, and required capabilities before mutation. Validate the selected entry's source and version and pass them as arguments to the documented commands; never evaluate an `installCommand` string as shell code. After the calling workflow's confirmation gate, install the pinned source first, then customize the installed recipe by the name that install or the recipe list reports into the approved repository-local output path — customize resolves installed recipes, not raw sources. Preserve attribution, remove irrelevant example behavior and local capability configuration, and prove the result against the new user's cases. Installing or copying a recipe is not behavioral proof.

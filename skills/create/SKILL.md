...
Each capability that reads outside content widens what can steer the agent. When the agent will retrieve documents, run tools against untrusted output, fetch pages, or delegate to a child agent, load the `agent-security-review` reference while the capability set is still being decided. It is cheaper to leave a capability out than to gate it later.

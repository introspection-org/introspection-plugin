...
Which route obtained the template decides who rewrites its identity. When the CLI's own scaffolding verb fetched it, adoption is part of that verb: it drops the template's history and origin, removes the template's runtime manifest, writes a new one under the agreed slug with its package path rebased for wherever the recipe landed, and renames the package. Read the result back to confirm it, but do not rewrite it again — re-authoring the manifest is how a correct package acquires a stale package path or a second manifest, and a repository with two manifests is ambiguous at deploy.

When the template arrived by ordinary Git instead, none of that has happened and the rename is yours: the package name, the runtime manifest and its filename, and anything else naming the template rather than the new recipe all have to move to the agreed slug before the recipe is proven. A recipe still carrying the template's runtime name will claim that name at deploy, where it is reserved per project.
